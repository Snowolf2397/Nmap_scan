#!/bin/bash
if [ $(id -u) -ne 0 ]; then
	echo -e "\n[!] Tienes que ejecutarlo como root el script -> (sudo $0)"
exit 1
fi
...
if [ "$(echo $?)" == "0" ]; then
	clear
    echo "S̴̟̞̼̓͑̚n̸̡͎͖̾͆̾o̵̢͇̾̚͠w̸̡̘͌̔o̵̘̞̫̐̔͝l̴̻̪̪̈́̔̽f̵͙͓̻̿͒͛"
  read -p "Introduce la direccion IP objetivo: " ip	
	while true; do
